...
Defines structure and training methods for building an auto-encoding neural net, in TensorFlow.
The net attempts to recover an input image in the output layer.
"""
...
from ae.utils.data import fill_feed_dict_ae, read_data_sets_pretraining
from ae.utils.data import read_data_sets, fill_feed_dict
from ae.utils.flags import FLAGS
from ae.utils.eval import loss_supervised, evaluation, do_eval_summary
from ae.utils.utils import tile_raster_images
...
class AutoEncoder(object):
    ...
    @property
    def shape(self):
        """ The shape of all layers of this autoencoder. """
        return self.__shape

    @property
    def num_hidden_layers(self):
        """ The number of hidden layers in this autoencoder. """
        return self.__num_hidden_layers

    @property
    def session(self):
        """ The tensorflow session for this autoencoder. """
        return self.__sess
    ...
    def _setup_variables(self):
        with tf.name_scope("autoencoder_variables"):
            for i in range(self.__num_hidden_layers + 1):
                ...
                max_a = 4.0 * tf.sqrt(6.0 / (w_shape[0] + w_shape[1]))
                w_init = tf.random_uniform(w_shape, -1 * max_a, max_a)
    ...
    def _w(self, step_n, suffix=""):
        return self[self._weights_str.format(step_n) + suffix]

    def _b(self, step_n, suffix=""):
        return self[self._biases_str.format(step_n) + suffix]

    def get_variables_to_init(self, step_n):
        ...
        Args:
          step_n: int giving step of training
        """
        assert step_n > 0
        assert step_n <= self.__num_hidden_layers + 1

        vars_to_init = [self._w(step_n), self._b(step_n)]

        if step_n <= self.__num_hidden_layers:
            vars_to_init.append(self._b(step_n, "_out"))

        if 1 < step_n <= self.__num_hidden_layers:
            vars_to_init.append(self._w(step_n - 1, "_fixed"))
            vars_to_init.append(self._b(step_n - 1, "_fixed"))
    ...
    def pretrain_net(self, input_pl, step_n, is_target=False):
        """Return net for step n training or target net

        Args:
          input_pl:  tensorflow placeholder of AE inputs
          step_n:    int specifying pretrain step
        ...
        """
        assert step_n > 0
        assert step_n <= self.__num_hidden_layers

        last_output = input_pl
        for i in range(step_n - 1):
            weight = self._w(i + 1, "_fixed")
            bias = self._b(i + 1, "_fixed")

            last_output = self._activate(last_output, weight, bias)

        if is_target:
            return last_output

        last_output = self._activate(last_output, self._w(step_n), self._b(step_n))

        out = self._activate(last_output, self._w(step_n), self._b(step_n, "_out"),
                             transpose_w=True)
...
loss_summaries = {} # pylint: disable=invalid-name
...
def loss_x_entropy(output, target):
    ...
    with tf.name_scope("xentropy_loss"):
        ...
        cross_entropy = ((tf.log(net_output_tf, name='log_output') * target_tf) +
                         (tf.log(1 - net_output_tf, name='loginv_output') * (1 - target_tf)))
        return -1 * tf.reduce_mean(tf.reduce_sum(cross_entropy, 1),
                                   name='xentropy_mean')


def main_unsupervised():
    """ Performs usupervised training of autoencoder, recovering the input at the output """
    with tf.Graph().as_default():
        ...
        ae = AutoEncoder(ae_shape, sess) # pylint: disable=invalid-name
        ...
        for i in range(len(ae_shape) - 2):
            layer_num = i + 1
            with tf.variable_scope("pretrain_{0}".format(layer_num)):
                ...
                target_ = tf.placeholder(dtype=tf.float32,
                                         shape=(FLAGS.batch_size, ae_shape[0]),
                                         name='ae_target_pl')
                layer = ae.pretrain_net(input_, layer_num)

                with tf.name_scope("target"):
                    target_for_loss = ae.pretrain_net(target_, layer_num, is_target=True)

                loss = loss_x_entropy(layer, target_for_loss)
                train_op, global_step = training(loss, learning_rates[i], i)

                summary_dir = pjoin(FLAGS.summary_dir, 'pretraining_{0}'.format(layer_num))
                summary_writer = tf.summary.FileWriter(summary_dir,
                                                       graph=sess.graph,
                                                       flush_secs=FLAGS.flush_secs)
                summary_vars = [ae["biases{0}".format(layer_num)],
                                ae["weights{0}".format(layer_num)]]
                ...
                vars_to_init = ae.get_variables_to_init(layer_num)
...
def main_supervised(ae): # pylint: disable=invalid-name
    """ Performs supervised training of output layer against labels """
    with ae.session.graph.as_default():
        ...
        summary_writer = tf.summary.FileWriter(pjoin(FLAGS.summary_dir, 'fine_tuning'),
                                               graph=sess.graph,
                                               flush_secs=FLAGS.flush_secs)
...
if __name__ == '__main__':
    AE = main_unsupervised() # pylint: disable=invalid-name
    main_supervised(AE)
